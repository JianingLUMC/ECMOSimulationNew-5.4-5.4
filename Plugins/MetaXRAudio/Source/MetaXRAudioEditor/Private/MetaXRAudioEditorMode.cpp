...
void FMetaXRAudioEditorMode::Enter() {
  ...
  TransformProxy->OnTransformChanged.AddLambda([this](UTransformProxy* Proxy, FTransform Transform) {
    if (!EditedComponent)
      return;
    if (EditedComponent->CachedPoints.IsEmpty())
      return;
    if (EditedComponent->SelectedPointIndex >= EditedComponent->CachedPoints.Num())
      return;

    EditedComponent->bHasCustomPoints = true;

    // The transform gizmo will provide a new position for the point in UE coordinates
    // We want to store all Cached Points in ovrAudio coordinates with no map transform applied
    FTransform MapTransform = EditedComponent->GetComponentTransform().Inverse();
    FVector& Point = EditedComponent->CachedPoints[EditedComponent->SelectedPointIndex];
    Point = MetaXRAudioUtilities::ToOVRVector(MapTransform.TransformPosition(Transform.GetLocation()));
  });
}
...
void FMetaXRAudioEditorMode::Tick(FEditorViewportClient* ViewportClient, float DeltaTime) {
  ...
  if (World) {
    for (TActorIterator<AActor> ActorItr(World); ActorItr; ++ActorItr) {
      UMetaXRAcousticMap* FoundComponent = ActorItr->FindComponentByClass<UMetaXRAcousticMap>();
      if (FoundComponent && FoundComponent != EditedComponent) {
        EditedComponent = FoundComponent;
        EditedComponent->SelectedPointIndex = MAX_int32;
        if (!EditedComponent->CachedPoints.IsEmpty() && EditedComponent->SelectedPointIndex < EditedComponent->CachedPoints.Num()) {
          // All cached points are stored in ovrAudio coordinates with no Acoustic Map transform applied
          // Here we want the transform applied and UE coordinates because the transform proxy will be visualized
          FTransform MapTransform = EditedComponent->GetComponentTransform();
          const FVector& Point = EditedComponent->CachedPoints[EditedComponent->SelectedPointIndex];
          const FTransform Transform(
              FQuat::Identity, MapTransform.TransformPosition(MetaXRAudioUtilities::ToUEVector(Point)), FVector(1.0f));
  ...
  if (EditedComponent) {
    const bool bHasSelectedPoint = EditedComponent->SelectedPointIndex < EditedComponent->CachedPoints.Num();
    bTransformGizmoVisible = EditedComponent ? (EditedComponent->bCustomPointsEnabled && bHasSelectedPoint) : false;
  }
...
bool FMetaXRAudioEditorMode::PerformRaycast(FEditorViewportClient* ViewportClient, const FVector2D& MousePosition) {
  ...
  if (FViewport* ActiveViewport = ViewportClient->Viewport) {
    ...
    for (int32 i = 0; i < EditedComponent->CachedPoints.Num(); i++) {
      // Cached Points are stored as ovrAudio coordinates with no Acoustic Map Transform
      // Here we want the transform applied and UE coordinates as this matches the mouse click specifications
      FTransform MapTransform = EditedComponent->GetComponentTransform();
      const FVector Point = MapTransform.TransformPosition(MetaXRAudioUtilities::ToUEVector(EditedComponent->CachedPoints[i]));
      ...
      if (FVector::DistSquared(ClosestPoint, Point) <= META_XR_AUDIO_MAP_SPHERE_RADIUS * META_XR_AUDIO_MAP_SPHERE_RADIUS) {
        EditedComponent->SelectedPointIndex = i;
