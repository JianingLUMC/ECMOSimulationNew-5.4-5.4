...
ovrAudioContext FMetaXRAudioContextManager::GetOrCreateSerializationContext(AActor* Parent) {
...
ovrAudioContext FMetaXRAudioContextManager::GetContext(AActor* actor, UWorld* World) {
  ovrAudioContext PluginContext = FMetaXRAudioLibraryManager::Get().GetPluginContext();
  if (PluginContext != nullptr) {
    return PluginContext;
  } else {
    FMetaXRAudioPlugin* Plugin = &FModuleManager::GetModuleChecked<FMetaXRAudioPlugin>("MetaXRAudio");
    if (Plugin == nullptr) {
      UE_LOG(LogAudio, Error, TEXT("Meta XR Audio Plugin is not loaded and therefore cannot get an audio context"));
      return nullptr;
    }

    // Note: getting the context from the spatializer, should we get it from the reverb instead?
    FString MetaXRSpatializerPluginName = Plugin->GetSpatializationPluginFactory()->GetDisplayName();
